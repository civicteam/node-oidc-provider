...
export default function getClaims(provider) {
  ...
  return class Claims {
    ...
    async result() {
      ...
      if (allowDynamicClaims) {
        // When allowDynamicClaims is true, include all available claims
        // plus any specifically requested claims from the filter
        const requestedClaims = Object.entries(this.filter)
          .map(([key, value]) => {
        ...
        // Include all available claims, not just those in the filter
        // but exclude rejected claims
        include = [...new Set([...Object.keys(available), ...requestedClaims])]
          .filter((claim) => !this.rejectedClaims.has(claim));
      } else {
        // Original behavior when allowDynamicClaims is false
        include = Object.entries(this.filter)
          .map(([key, value]) => {
            if (value === null || isPlainObject(value)) {
              return key;
            }
