...
async function idTokenHandler(ctx) {
  ...
  const originalClaims = ctx.oidc.claims?.id_token || {};
  const claimsForAccount = allowDynamicClaims ? originalClaims : filterClaims(ctx.oidc.claims, 'id_token', ctx.oidc.grant, ctx.oidc.provider);
  ...
  const idToken = new ctx.oidc.provider.IdToken({
    ...await getCtxAccountClaims(ctx, 'id_token', scope, claimsForAccount, rejected),
    acr: ctx.oidc.acr,
    amr: ctx.oidc.amr,
  ...
  const claims = allowDynamicClaims ? originalClaims : filterClaims(ctx.oidc.claims, 'id_token', ctx.oidc.grant, ctx.oidc.provider);
