import { InvalidTarget, InvalidGrant } from './errors.js';
...
async function tokenHandler(ctx) {
  ...
  const { configuration } = instance(ctx.oidc.provider);
  ...
  if (configuration.relaxImplicitGrantRequirement && !ctx.oidc.client.grantTypeAllowed('implicit')) {
    throw new InvalidGrant('client not allowed to use implicit grant');
  }
