...
export default async function resumeAction(allowList, resumeRouteName, ctx, next) {
  ...
  // If we have a custom shouldWriteCookies function in config that returns false, it means we don't want cookies to hamper authentication and so we bypass this session mismatch check.
