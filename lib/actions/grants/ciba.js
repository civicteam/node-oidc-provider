...
export const handler = async function cibaHandler(ctx) {
  ...
  if (request.scopes.has('openid')) {
    ...
    const originalClaims = request.claims || {};
    const claimsForAccount = allowDynamicClaims ? originalClaims : filterClaims(request.claims, 'id_token', grant, ctx.oidc.provider);
    
    const rejected = grant.getRejectedOIDCClaims();
    const token = new IdToken({
      ...await getCtxAccountClaims(ctx, 'id_token', request.scope, claimsForAccount, rejected),
      ...{
        acr: request.acr,
        amr: request.amr,
    ...
    const claims = allowDynamicClaims ? originalClaims : filterClaims(request.claims, 'id_token', grant, ctx.oidc.provider);
