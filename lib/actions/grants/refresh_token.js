...
export const handler = async function refreshTokenHandler(ctx) {
  ...
  if (scope.has('openid')) {
    ...
    const originalClaims = refreshToken.claims || {};
    const claimsForAccount = allowDynamicClaims ? originalClaims : filterClaims(refreshToken.claims, 'id_token', grant, ctx.oidc.provider);
    
    const rejected = grant.getRejectedOIDCClaims();
    const token = new IdToken(({
      ...await getCtxAccountClaims(ctx, 'id_token', [...scope].join(' '), claimsForAccount, rejected),
      acr: refreshToken.acr,
      amr: refreshToken.amr,
    ...
    const claims = allowDynamicClaims ? originalClaims : filterClaims(refreshToken.claims, 'id_token', grant, ctx.oidc.provider);
