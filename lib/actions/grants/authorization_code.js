...
export const handler = async function authorizationCodeHandler(ctx) {
  ...
  if (code.scopes.has('openid')) {
    ...
    const originalClaims = code.claims || {};
    const claimsForAccount = allowDynamicClaims ? originalClaims : filterClaims(code.claims, 'id_token', grant, ctx.oidc.provider);
    
    const rejected = grant.getRejectedOIDCClaims();
    const token = new IdToken({
      ...await getCtxAccountClaims(ctx, 'id_token', code.scope, claimsForAccount, rejected),
      acr: code.acr,
      amr: code.amr,
    ...
    const claims = allowDynamicClaims ? originalClaims : filterClaims(code.claims, 'id_token', grant, ctx.oidc.provider);
