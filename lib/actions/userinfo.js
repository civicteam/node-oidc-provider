...
export default [
  ...
  async function respond(ctx) {
    const { allowDynamicClaims } = instance(ctx.oidc.provider).configuration;
    
    // When allowDynamicClaims is true, pass original claims to account.claims() and filter after
    // When allowDynamicClaims is false, filter claims before passing to account.claims()
    const originalClaims = ctx.oidc.accessToken.claims || {};
    const claimsForAccount = allowDynamicClaims ? originalClaims : filterClaims(ctx.oidc.accessToken.claims, 'userinfo', ctx.oidc.grant, ctx.oidc.provider);
    ...
    // Set the mask for filtering - when allowDynamicClaims is true, don't filter
    const claims = allowDynamicClaims ? originalClaims : filterClaims(ctx.oidc.accessToken.claims, 'userinfo', ctx.oidc.grant, ctx.oidc.provider);

    if (client.userinfoSignedResponseAlg || client.userinfoEncryptedResponseAlg) {
      const token = new ctx.oidc.provider.IdToken(
        await getCtxAccountClaims(ctx, 'userinfo', scope, claimsForAccount, rejected),
        { ctx },
      );
