...
export default (provider) => class Session extends hasFormat(provider, 'Session', instance(provider).BaseModel) {
  ...
  static async getSessionFromIdTokenHint(ctx) {
    try {
      const clientId = ctx.oidc?.params?.client_id || ctx.request?.query?.client_id;
      if (!clientId) {
      ...
      const idTokenJwt = ctx.oidc?.params?.id_token_hint || ctx.request?.query?.id_token_hint;;
      ...
      const session = await this.findByUid(sessionId);
      if (!sessionId) {
        return undefined;
      }
  ...
  static async get(ctx) {
    ...
    if (!session && cookielessFallbackEnabled) {
      ...
      const sessionUid = this.getSessionFromIdTokenHint(ctx);
      if (sessionUid) {
        session = await this.findByUid(sessionUid);
      }
    }
